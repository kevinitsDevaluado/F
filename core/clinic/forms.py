...
class MascotsForm(ModelForm):
    ...
    class Meta:
        model = Mascots
        fields = 'name', 'client', 'color', 'image', 'typepet', 'breed', 'gender', 'birthdate', 'observation'
        widgets = {
            'name': forms.TextInput(attrs={'placeholder': 'Ingrese un nombre'}),
            ...
            'breed': forms.Select(attrs={'class': 'form-control select2', 'style': 'width: 100%;'}),
    ...
    typepet = forms.ModelChoiceField(widget=forms.Select(
        attrs={
            'class': 'form-control select2',
            'style': 'width: 100%;'
        }
    ), queryset=TypePet.objects.all(), label='Tipo de Animal', required=True)
...
class ProductForm(ModelForm):
    ...
    class Meta:
        model = Product
        fields = 'name', 'producttype', 'image', 'price', 'desc',
        widgets = {
            'name': forms.TextInput(attrs={'placeholder': 'Ingrese un nombre'}),
